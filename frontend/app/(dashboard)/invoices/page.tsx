...
import { Receipt, Plus, Loader2, Bell, ChevronUp, ChevronDown } from "@/components/icons";
...
export default function InvoicesPage() {
  ...
  const [manageInvoice, setManageInvoice] = useState<InvoiceWithPack | null>(null);
  ...
  useEffect(() => {
    revenue.getConnectStatus().then(setConnectStatus).catch(() => setConnectStatus({ connected: false, onboarding_complete: false }));
  }, []);
    ...
    <div className="p-8 w-full max-w-[1400px] mx-auto">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {sentNotPaid > 0 && (
            <Button variant="outline" size="sm" onClick={handleRemindOverdue} disabled={reminding}>
              {reminding ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Bell className="h-4 w-4" /> Remind overdue</>}
            </Button>
          )}
                ...
                <form onSubmit={handleCreate}>
                  <DialogBody className="space-y-4">
                    <div>
                      ...
                      <Select id="create-pack" value={createPackId} onChange={(e) => setCreatePackId(e.target.value)} className="mt-1">
                        {packs.map((p) => (
                    ...
                    <div>
                      ...
                      <Input id="amount" type="text" value={formAmount} onChange={(e) => setFormAmount(e.target.value)} placeholder="e.g. 1500.00" required className="mt-1" />
                    </div>
                    <div>
                      <Label htmlFor="currency">Currency</Label>
                      <Select id="currency" value={formCurrency} onChange={(e) => setFormCurrency(e.target.value)} className="mt-1">
                    ...
                    <div>
                      ...
                      <Input id="due_date" type="date" value={formDueDate} onChange={(e) => setFormDueDate(e.target.value)} className="mt-1" />
                    </div>
                  </DialogBody>
                  ...
                  {filteredInvoices.map((invoice) => (
                    <TableRow key={invoice.id}>
                      ...
                      <TableCell className="font-medium">
                        ...
                        {Number.isNaN(Number(invoice.amount)) ? invoice.amount : Number(invoice.amount).toLocaleString()}
                      ...
                      <TableCell className="text-muted-foreground whitespace-nowrap">
                        {formatTableDate(invoice.due_date ?? invoice.created_at)}
                      </TableCell>
                      <TableCell>
                        <span
                          className={cn(
                            "inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium",
                            invoice.status === "paid" && "bg-green-500/20 text-green-700 dark:text-green-400",
                            invoice.status === "overdue" && "bg-destructive/20 text-destructive",
                            invoice.status === "sent" && "bg-amber-500/20 text-amber-700 dark:text-amber-400",
                            invoice.status === "draft" && "bg-muted text-muted-foreground"
                          )}
                        >
                          {invoice.status === "sent" ? "Pending" : invoice.status === "paid" ? "Paid" : invoice.status === "overdue" ? "Overdue" : invoice.status}
                        </span>
                      </TableCell>
      ...
      <Dialog open={!!manageInvoice} onOpenChange={(open) => { if (!open) { setManageInvoice(null); setPublishedLink(null); } }}>
        {manageInvoice && (
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Manage invoice</DialogTitle>
              <DialogDescription>
                {formatInvoiceId(manageInvoice.id)} · {manageInvoice.currency === "USD" && "$"}
                {Number.isNaN(Number(manageInvoice.amount)) ? manageInvoice.amount : Number(manageInvoice.amount).toLocaleString()}
                {manageInvoice.currency !== "USD" && ` ${manageInvoice.currency}`}
              </DialogDescription>
              <DialogClose onClose={() => { setManageInvoice(null); setPublishedLink(null); }} />
            </DialogHeader>
            <DialogBody>
              <div className="flex flex-col gap-3">
                {manageInvoice.stripe_hosted_url ? (
                  <div className="space-y-2">
                    <p className="text-sm text-muted-foreground">Payment link ready. Share it with your client so they can pay online.</p>
                    <div className="flex flex-wrap gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => {
                          navigator.clipboard.writeText(manageInvoice.stripe_hosted_url!);
                        }}
                      >
                        Copy link
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => window.open(manageInvoice.stripe_hosted_url!, "_blank")}
                      >
                        Open link
                      </Button>
                    </div>
                  ...
                  <div className="space-y-2">
                    <p className="text-sm text-muted-foreground">Create a shareable payment link. Your client can pay by card on Stripe.</p>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={async () => {
                        ...
                        setError(null);
                        try {
                          const res = await revenue.publishInvoice(manageInvoice.id);
                          setPublishedLink(res.payment_link);
                          await fetchAll();
                          setManageInvoice((prev) => prev ? { ...prev, stripe_hosted_url: res.payment_link, stripe_invoice_id: res.stripe_invoice_id } : null);
                        } catch (e) {
                    ...
                    >
                      {publishLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Get payment link"}
                    </Button>
                    {publishedLink && (
                      <div className="flex flex-wrap gap-2 pt-2">
                        <Button variant="outline" size="sm" onClick={() => navigator.clipboard.writeText(publishedLink)}>Copy link</Button>
                        <Button variant="outline" size="sm" onClick={() => window.open(publishedLink, "_blank")}>Open link</Button>
                      </div>
                    )}
                  </div>
                ) : (
                  <p className="text-sm text-muted-foreground">
                    <Link href="/settings" className="text-primary hover:underline">Connect Stripe</Link> in Settings to create payment links.
                  </p>
                )}
                ...
                {manageInvoice.status === "draft" && (
                  ...
                  >
                    {editingId === manageInvoice.id ? <Loader2 className="h-4 w-4 animate-spin" /> : "Mark sent"}
                  </Button>
                )}
                {(manageInvoice.status === "sent" || manageInvoice.status === "overdue") && (
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => {
                    ...
                    disabled={editingId === manageInvoice.id}
                  >
                    {editingId === manageInvoice.id ? <Loader2 className="h-4 w-4 animate-spin" /> : "Mark paid"}
                  </Button>
                )}
