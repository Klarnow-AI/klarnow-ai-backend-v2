...
def _upload_logo_variant(
    *,
    ...
    path_key = f"logos/{pack_id}/{asset_id}{suffix}.{extension}"
    upload_result = upload_file(path_key, image_bytes, content_type=mime_type)
    ...
    # ``upload_file`` returns ``{record_id}/{filename}`` — that's the key shape
    # the PocketBase file API actually expects. Passing our own ``path_key``
    # here would produce a URL like ``/api/files/storage_files/logos/...`` that
    # 404s because ``logos`` isn't a record id.
    return get_asset_url(uploaded, expires_in=86400 * 7) or f"key:{uploaded}"
...
def render_brand_image(
    prompt_text: str,
    *,
    storage_prefix: str,
    aspect_ratio: str = OPENROUTER_IMAGE_ASPECT_RATIO,
    image_size: str = OPENROUTER_IMAGE_SIZE,
) -> dict[str, str | None]:
    """Render a single image from a prompt and upload it to storage.

    Generic counterpart to :func:`generate_logo_with_openrouter` — same
    OpenRouter call path, but without the transparent-background post-pass
    that only makes sense for logos. Callers (creative assets, hero images,
    etc.) can pick their own aspect ratio and storage namespace.

    Returns ``{"image_url": <url>, "mime_type": <mime>}`` on success, or
    raises :class:`BadRequestError` if generation is disabled, the provider
    cooldown is active, or the model fails to return an image.
    """
    settings = get_settings()
    if not has_openai_compatible_provider() or not settings.ai_logo_generation_enabled:
        raise BadRequestError(
            "Image generation is disabled. "
            "Set OPENROUTER_API_KEY and AI_LOGO_GENERATION_ENABLED=true to generate images."
        )
    ...
    # Reuse the logo call helper directly but override aspect ratio per call.
    # We don't want to touch _request_logo_image_generation's retry-on-modality
    # behavior, so we inline the loop here rather than forking it.
    message_payload = [{"role": "user", "content": prompt_for_model}]
    extra_body_attempts = get_image_generation_extra_body_attempts(
        model_name,
        ...
        try:
            ...
            if not has_fallback or not is_unsupported_output_modalities_error(exc):
                if _is_provider_auth_or_quota_error(exc):
                    ...
                    raise BadRequestError(reason) from None
                logger.warning(
                    "render_brand_image: OpenRouter image call failed: %s", exc, exc_info=True
                )
                raise BadRequestError(
    ...
    if response is None:
        raise BadRequestError(
            f"Image generation failed before any OpenRouter request succeeded: {last_exc!s}"
        )

    image_payload = _extract_generated_image_payload(response)
    if not image_payload:
        logger.warning(
            "render_brand_image: OpenRouter response had no image payload; %s",
            _summarize_generated_image_response(response),
        )
        raise BadRequestError("Image generation did not return an image.")
    ...
    if payload_kind == "data_url":
        ...
        image_bytes, mime_type = _download_generated_image(
            payload_value, payload_mime_type
        )
    else:
    ...
    path_key = f"{storage_prefix.rstrip('/')}/{asset_id}.{extension}"
    upload_result = upload_file(path_key, image_bytes, content_type=mime_type)
    uploaded = asyncio.run(upload_result) if inspect.isawaitable(upload_result) else upload_result
    if not uploaded:
        logger.warning("render_brand_image: upload_file returned falsy for key=%s", path_key)
        raise BadRequestError("Image was generated but storage upload failed.")
    # Use the storage key PocketBase just gave us ({record_id}/{filename}),
    # not our path_key — otherwise the generated URL points at a nonexistent
    # record and the browser shows a broken image.
    image_url = get_asset_url(uploaded, expires_in=86400 * 7) or f"key:{uploaded}"
    return {"image_url": image_url, "mime_type": mime_type}
